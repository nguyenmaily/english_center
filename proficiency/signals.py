...
def update_proficiency_from_final_test(sender, instance, created, **kwargs):
    ...
    try:
        ...
        # Chỉ xử lý final test hoặc placement test
        ...
        # Tạo hoặc cập nhật StudentCertificate
        ...
        if created:
            logger.info(f"Created {test_type_name} certificate for student {student.id}, skill_group {skill_group}, score {total_score}")
        else:
            logger.info(f"Updated {test_type_name} certificate for student {student.id}, skill_group {skill_group}, score {total_score}")
